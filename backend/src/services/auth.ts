...
import { findByEmail, createUser, findById, toSafeUser, type User, type SafeUser } from './userStore.js';
...
export async function login(email: string, password: string): Promise<{ token: string; user: SafeUser }> {
  ...
  // JWT 토큰 생성 (사용자 ID와 이메일을 페이로드에 담음)
